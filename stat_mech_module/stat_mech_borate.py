...
def B_onedraw(w, start_conc, draw_size, back=False):
    ...
    if ((B4_s+B2_s+B1_s*2+B0_s*3)/(B3_s+B4_s+B2_s+B1_s*+B0_s)) < 0.428:
        B4_B2 = 1
    else:
        B4_B2 = 0
...
#                w1 = [1, 0.01, 0.001]
    ...
    for i in draw_ar:
        ...
        p_B3 = B3[-1]*w_B3[i] / (B3[-1]*w_B3[i] + B4[-1]*w_B4[i] + B2[-1]*w_B2[i] + B1[-1]*w_B1[i])
        p_B4 = B4[-1]*w_B4[i] / (B3[-1]*w_B3[i] + B4[-1]*w_B4[i] + B2[-1]*w_B2[i] + B1[-1]*w_B1[i])
        p_B2 = B2[-1]*w_B2[i] / (B3[-1]*w_B3[i] + B4[-1]*w_B4[i] + B2[-1]*w_B2[i] + B1[-1]*w_B1[i])
        p_B1 = B1[-1]*w_B1[i] / (B3[-1]*w_B3[i] + B4[-1]*w_B4[i] + B2[-1]*w_B2[i] + B1[-1]*w_B1[i])
        ...
        if B2[-1] + (p_B3*(1-B4_B2[i])) + (p_B4 * CB3) + p_B4 - p_B2 - (p_B4 * CB2) < 0:
            next_B2 = 0
        else:
            next_B2 = B2[-1] + (p_B3*(1-B4_B2[i])) + (p_B4 * CB3) + p_B4 - p_B2 - (p_B4 * CB2)
...
def B_SSE(w1, data, frac = None, s_plt = False, s_dat = False, p = False):
    ...
    if s_plt is False and p is True:
        plt.plot(M2O, B3, 'r-', M2O, B4, 'k-', M2O, B2, 'b-', M2O, B1, 'g-', M2O, B0, 'y-',)
...
def B_engine(fil, data, it = 10):
    ...
    res = scipy.optimize.basinhopping(B_SSE, w0, niter=it, T=2.0, stepsize=1, 
