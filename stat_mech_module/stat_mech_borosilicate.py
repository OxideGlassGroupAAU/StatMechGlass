...
def model(w1):
    ...
    H1 = [0, 8.4137, 7.4205, 35.31424822, 0, 14.097, 22.898, 27.098]

    w_Tf = [
        math.exp(-(H1[0] / (0.008314462 * w1[2]))),
        math.exp(-(H1[1] / (0.008314462 * w1[2]))),
        math.exp(-(H1[2] / (0.008314462 * w1[2]))),
        math.exp(-(H1[3] / (0.008314462 * w1[2]))),
        math.exp(-(H1[4] / (0.008314462 * w1[2]))),
        math.exp(-(H1[5] / (0.008314462 * w1[2]))),
        math.exp(-(H1[6] / (0.008314462 * w1[2]))),
        math.exp(-(H1[7] / (0.008314462 * w1[2]))),
    ]
    ...
    for i in list(range(len(k_si_data))):
        ...
        for i2 in draw_ar:
            next_mod = (
                (draw_ar[i2])
                / (
                    (k_number * 0.5 / (1 + (k_number * 0.5))) * 200
                    + ((1 / (1 + (k_number * 0.5))) * 100)
                    + draw_ar[i2]
                )
            ) * 100
            M2O.append(next_mod)
        ...
        Q4 = [
            ((k_number * 0.5) / (1 + (k_number * 0.5))) * 100,
        ]
        Q3 = [
            0,
        ]
...
w_Tf = [
    math.exp(-(H1[0] / (0.008314462 * res.x[2]))),
    math.exp(-(H1[1] / (0.008314462 * res.x[2]))),
    math.exp(-(H1[2] / (0.008314462 * res.x[2]))),
    math.exp(-(H1[3] / (0.008314462 * res.x[2]))),
    math.exp(-(H1[4] / (0.008314462 * res.x[2]))),
    math.exp(-(H1[5] / (0.008314462 * res.x[2]))),
    math.exp(-(H1[6] / (0.008314462 * res.x[2]))),
    math.exp(-(H1[7] / (0.008314462 * res.x[2]))),
]
...
m_data_si_plt_kval = np.column_stack(
    [Q4_data, Q4_mp, Q3_data, Q3_mp, Q2_data, Q2_mp, Q1_data, Q1_mp]
)
m_data_b_plt_kval = np.column_stack([N4_data, N4_mp])
np.savetxt(
    os.path.join(f"{fil}_data", f"Model_Qn_v_data_Qn_{fil}.csv"), m_data_si_plt_kval
)
np.savetxt(
    os.path.join(f"{fil}_data", f"Model_Bn_v_data_Qn_{fil}.csv"), m_data_b_plt_kval
)
#
...
for i4 in draw_ar:
    ...
    if (
        B2_k2[-1] + (p_B3 * (1 - B4_B2[i4])) + (p_B4 * CB3) + p_B4 - p_B2 - (p_B4 * CB2)
        < 0
    ):
        next_B2 = 0
    else:
...
m_Qdata_k2 = np.column_stack(
    [mod_si_data_k2, Q4_data_k2, Q3_data_k2, Q2_data_k2, Q1_data_k2, Q0_data_k2]
)
...
m_Qdata_k4 = np.column_stack(
    [mod_si_data_k05, Q4_data_k05, Q3_data_k05, Q2_data_k05, Q1_data_k05, Q0_data_k05]
)
...
for i4 in draw_ar:
    ...
    if (
        B2_k6[-1] + (p_B3 * (1 - B4_B2[i4])) + (p_B4 * CB3) + p_B4 - p_B2 - (p_B4 * CB2)
        < 0
    ):
        next_B2 = 0
    else:
...
m_Qdata_k6 = np.column_stack(
    [mod_si_data_k6, Q4_data_k6, Q3_data_k6, Q2_data_k6, Q1_data_k6, Q0_data_k6]
)
